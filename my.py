...
while cap.isOpened():
    ...
    if result.multi_hand_landmarks is not None:
        ...
        for res in result.multi_hand_landmarks:
            ...
            idx = int(results[0][0]) #idx는 label
            ...
            cv2.putText(img, text='please', org=(int(img.shape[1] / 2), 100), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=2, color=(255, 255, 255), thickness=3)
            mp_drawing.draw_landmarks(img, res, mp_hands.HAND_CONNECTIONS)
            if len(rps_result) >= 2:
                text=  rps_result[1]['rps']+rps_result[0]['rps']
